...
func (a *App) authorizeJoinFields(channel helper.StringInt64, token string) (int64, int64, int64, *int64, int64, string, string, string, error) {
	if token == "" || channel == 0 {
		return 0, 0, 0, nil, 0, "", "", "", fmt.Errorf("expected join")
	}
	uid, tokStreamID, voiceChannelID, tokGuild, perms, role, sourceType, audioMode, err := a.validateJoinToken(token)
	if err != nil {
		return 0, 0, 0, nil, 0, "", "", "", fmt.Errorf("unauthorized")
	}
	if tokStreamID != 0 && tokStreamID != int64(channel) {
		return 0, 0, 0, nil, 0, "", "", "", fmt.Errorf("unauthorized")
	}
	return uid, int64(channel), voiceChannelID, tokGuild, perms, role, sourceType, audioMode, nil
}
...
func supportedVoiceGatewayCodecs(allowAV1 bool) []voicev2.Codec {
	...
	codecs = append(codecs,
		voicev2.Codec{Name: "VP9", Type: "video", PayloadType: 98, RTXPayloadType: 99, Priority: 3000},
		voicev2.Codec{Name: "VP8", Type: "video", PayloadType: 96, RTXPayloadType: 97, Priority: 2000},
		voicev2.Codec{Name: "H264", Type: "video", PayloadType: 102, RTXPayloadType: 103, Priority: 1500},
	)
	return codecs
}
...
func (a *App) signalV2ReadyPayload(perms int64) voicev2.Ready {
	return voicev2.Ready{
		...
		MaxVideoBitrateKbps: a.cfg.MaxVideoBitrateKbps,
		Experiments:         []string{},
...
func (a *App) finalizeSignalV2Session(session *signalV2Session) {
	...
	if joinNotified {
		a.notifyUserLeave(session.ctx, userID, channelID, voiceChannelID, guildID, sessionID, "ended")
	}
	if pc != nil {
		_ = pc.Close()
	}
