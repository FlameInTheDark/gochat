...
func (a *App) handleSignalWSV1(c *websocket.Conn) {
	...
	uid, channelID, voiceChannelID, guildID, perms, role, sourceType, audioMode, err := a.authorizeJoin(joinEnv)
	if err != nil {
		signalSpan.RecordError(err)
		signalSpan.SetStatus(codes.Error, err.Error())
	...
	if role == streammeta.RolePublisher {
		a.notifyUserJoin(sessionCtx, uid, channelID, voiceChannelID, guildID, sourceType, audioMode, legacySessionID)
		defer a.notifyUserLeave(observability.BackgroundFromContext(sessionCtx), uid, channelID, voiceChannelID, guildID, legacySessionID, "ended")
	}
...
func (a *App) authorizeJoin(env rtcJoinEnvelope) (int64, int64, int64, *int64, int64, string, string, string, error) {
	...
	uid, tokStreamID, voiceChannelID, tokGuild, perms, role, sourceType, audioMode, err := a.validateJoinToken(env.D.Token)
	if err != nil {
		return 0, 0, 0, nil, 0, "", "", "", fmt.Errorf("unauthorized")
	}
	if tokStreamID != 0 && tokStreamID != int64(env.D.Channel) {
		return 0, 0, 0, nil, 0, "", "", "", fmt.Errorf("unauthorized")
	}
	return uid, int64(env.D.Channel), voiceChannelID, tokGuild, perms, role, sourceType, audioMode, nil
}
