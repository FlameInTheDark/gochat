...
func (a *App) validateJoinToken(token string) (int64, int64, int64, bool, error) {
	...
	// Ensure token type and audience are correct.
	if claims.TokenType != "sfu" || !containsString(claims.Audience, "sfu") {
		return 0, 0, 0, false, fmt.Errorf("aud/typ mismatch")
	}

	return claims.UserID, claims.ChannelID, claims.Perms, claims.Moved, nil
}
