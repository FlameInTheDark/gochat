...
func NewSearch(addresses []string, tlsSkip bool, username, password string) (*Search, error) {
	...
	if err != nil {
		finishExists(err)
		return nil, fmt.Errorf("failed to check if bots index exists: %w", err)
	}
	finishExists(nil)
	...
	if res.StatusCode != http.StatusOK {
		...
		data, err := json.Marshal(defaultBotsIndex)
		if err != nil {
			finishCreate(err)
			return nil, err
		}
		...
		if err == nil && create != nil {
			...
			if create.IsError() {
				...
				err = fmt.Errorf("create bots index response error: %s", string(body))
			}
		}
		finishCreate(err)
		if err != nil {
...
func (s *Search) IndexBot(ctx context.Context, bot Bot) error {
	if s.osc == nil {
		return fmt.Errorf("opensearch client is not initialized")
	}
	if !bot.Public || bot.Disabled {
		return s.DeleteBot(ctx, bot.BotUserID)
	}
	...
	ctx, end := observability.StartDependencySpan(ctx, "opensearch", "index", indexName, attribute.Int64("bot.user_id", bot.BotUserID))
	res, err := s.osc.Index(
		indexName,
		bytes.NewReader(data),
		s.osc.Index.WithDocumentID(fmt.Sprintf("%d", bot.BotUserID)),
	...
	if res.IsError() {
		...
		err = fmt.Errorf("index bot response error: %s", string(body))
		end(err)
		return err
	}
	end(nil)
...
func (s *Search) DeleteBot(ctx context.Context, botUserID int64) error {
	if s.osc == nil {
		return fmt.Errorf("opensearch client is not initialized")
	}

	ctx, end := observability.StartDependencySpan(ctx, "opensearch", "delete", indexName, attribute.Int64("bot.user_id", botUserID))
	res, err := s.osc.Delete(indexName, fmt.Sprintf("%d", botUserID), s.osc.Delete.WithContext(ctx), s.osc.Delete.WithRefresh("wait_for"))
	if err != nil {
		end(err)
		return err
	}
	...
	if res.IsError() && res.StatusCode != http.StatusNotFound {
		body, _ := io.ReadAll(res.Body)
		err = fmt.Errorf("delete bot response error: %s", string(body))
		end(err)
		return err
	}
	end(nil)
...
func (s *Search) SearchBots(ctx context.Context, req SearchRequest) (*Results, error) {
	...
	if res.IsError() {
		...
		body, _ := io.ReadAll(res.Body)
		err = fmt.Errorf("search bots response error: %s", string(body))
		end(err)
		return nil, err
	}
	...
	for _, hit := range sr.Hits.Hits {
		ids = append(ids, hit.Source.BotUserID)
	}
	end(nil)
...
func (s *Search) SearchTags(ctx context.Context, query string, limit int) ([]string, error) {
	...
	ctx, end := observability.StartDependencySpan(ctx, "opensearch", "search_bot_tags", indexName)
	res, err := s.osc.Search(
		s.osc.Search.WithContext(ctx),
		s.osc.Search.WithIndex(indexName),
	...
	if res.IsError() {
		...
		body, _ := io.ReadAll(res.Body)
		err = fmt.Errorf("search bot tags response error: %s", string(body))
		end(err)
		return nil, err
	}
...
func buildOSQuery(req SearchRequest) ([]byte, error) {
	...
	osreq := osSearchRequest{
		From:   req.From,
		Size:   req.Size,
		Source: []string{"bot_user_id"},
	}
	osreq.Query.Bool.Filter = append(osreq.Query.Bool.Filter,
		osSearchQuery{Term: map[string]any{"public": true}},
		osSearchQuery{Term: map[string]any{"disabled": false}},
	)
	...
	if query != "" {
		osreq.Query.Bool.Must = append(osreq.Query.Bool.Must, osSearchQuery{MultiMatch: map[string]any{
			"query":  query,
			"fields": []string{"name^3", "tags^2", "description", "bio"},
		}})
	}
	...
	case SortPopularity:
		osreq.Sort = []map[string]any{
			{"installs_count": SortOrder{Order: "desc", Missing: "_last"}},
			{"_score": SortOrder{Order: "desc"}},
			{"bot_user_id": SortOrder{Order: "asc"}},
		}
		...
		"query": map[string]any{
			"bool": map[string]any{
				"filter": []map[string]any{
					...
					{"term": map[string]any{"disabled": false}},
				},
			},
		},
		"aggs": map[string]any{
