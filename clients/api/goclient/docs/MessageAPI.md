...
[**MessageChannelChannelIdThreadPost**](MessageAPI.md#MessageChannelChannelIdThreadPost) | **Post** /message/channel/{channel_id}/thread | Create thread in channel
...
## MessageChannelChannelIdThreadPost

> DtoChannel MessageChannelChannelIdThreadPost(ctx, channelId).Request(request).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.MessageAPI.MessageChannelChannelIdThreadPost(context.Background(), channelId).Request(request).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MessageAPI.MessageChannelChannelIdThreadPost``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `MessageChannelChannelIdThreadPost`: DtoChannel
	fmt.Fprintf(os.Stdout, "Response from `MessageAPI.MessageChannelChannelIdThreadPost`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiMessageChannelChannelIdThreadPostRequest struct via the builder pattern
